...
class economy(commands.Cog):
        ...
        if user_id == 1014344645020495942:
            if id == None:
                ...
                for row in results:
                    ...
                    embed.add_field(name=id, value=f"Name: {item_name}", inline=False)
    ...
    async def ecogive(self, ctx, *, member: discord.Member, money: int):
        ...
        result = self.db_cursor_economy.execute(query, (member_id, server_id)).fetchone()
        ...
        if result:
            ...
            self.db_cursor_economy.execute(query, (new_money, member_id, server_id))
            self.db_connection_economy.commit()
            ...
            self.db_cursor_economy.execute(query, (member_id, money, 1, server_id))
            self.db_connection_economy.commit()
    ...
    async def worktimer(self, ctx, *, worktime=None):
        ...
        result = self.db_cursor_worktime.execute(query, (server_id,)).fetchone()
        ...
        if worktime is None:
            ...
            if result:
                ...
                self.db_cursor_worktime.execute(update_query, (worktime, server_id))
                self.db_connection_worktime.commit()
                await ctx.send(f"Worktime was set to {worktime} minutes")
            
            else:
                insert_query = "INSERT INTO worktime (worktime, server_id) VALUES (?, ?)"
                self.db_cursor_worktime.execute(insert_query, (worktime, server_id))
                self.db_connection_worktime.commit()
                await ctx.send(f"Worktime was set to {worktime} minutes")
    ...
    async def work(self, ctx):
        ...
        lul = self.db_connection_worktime.execute(query3, (server_id,)).fetchone()
        if lul:
        ...
        for row in result:
            ...
            embed.add_field(name="Duration of work", value=duration_str, inline=False)
            embed.add_field(name="Remaining time", value=remaining_time_str, inline=False)
        ...
        if result:
            ...
            query = "SELECT * FROM economy WHERE user_id = ? AND server_id = ?"
            result = self.db_cursor_economy.execute(query, (user_id, server_id)).fetchone()
            ...
            if result:
                ...
                self.db_cursor_economy.execute(query, (money, user_id, server_id))
                self.db_connection_economy.commit()
                self.db_cursor_economy.execute(query2, (level, user_id, server_id))
                self.db_connection_economy.commit()

            else:
                query = "INSERT INTO economy (user_id, money, level, server_id) VALUES (?, ?, ?, ?)"
                self.db_cursor_economy.execute(query, (user_id, geld, 1,  server_id))
                self.db_connection_economy.commit()
            

            query = "INSERT INTO economytimer (user_id, reason, timestamp, expiration, server_id) VALUES (?, ?, ?, ?, ?)"
            self.db_cursor_economytimer.execute(query, (user_id, ok, timestamp, expiration_time, server_id))
            self.db_connection_economytimer.commit()
    ...
    async def shop(self, ctx):
        ...
        if results:
            ...
            for row in results:
                ...
                embed.add_field(name=item_name, value=f"Price: {item_price} Bits", inline=False)
    ...
    async def removemoney(self, ctx, member: discord.Member, bits: int):
        ...
        if result:
            ...
            if bits > money1:
                ...
                query = "UPDATE economy SET money = ? WHERE user_id = ? AND server_id = ?"
                self.db_cursor_economy.execute(query, (money_after_payment1, member_id, server_id))
                self.db_connection_economy.commit()
    ...
    async def pay(self, ctx, member: discord.Member, bits: int):
        ...
        result = self.db_cursor_economy.execute(query, (user_id, server_id)).fetchone()
        result2 = self.db_cursor_economy.execute(query2, (member_id, server_id)).fetchone()
        if result:
            ...
            if money1 < bits:
                ...
                query = "UPDATE economy SET money = ? WHERE user_id = ? AND server_id = ?"
                self.db_cursor_economy.execute(query, (money_after_payment1, user_id, server_id))
                self.db_connection_economy.commit()
                await ctx.send(f"Paying {bits} to {member.mention}")

                if result2:
                    query2 = "UPDATE economy SET money = ? WHERE user_id = ? AND server_id = ?"
                    self.db_cursor_economy.execute(query2, (money_after_payment2, member_id, server_id))
                    self.db_connection_economy.commit()
                    ...
                    self.db_cursor_economy.execute(query2, (member_id, money_after_payment2, level2,  server_id))
                    self.db_connection_economy.commit()
                    await ctx.send(f"Removing bits from <@{user_id}>...")
                    await ctx.send(f"Payment was successful")
    ...
    async def buy(self, ctx, name: str):
        ...
        if result:
            ...
            result = self.db_cursor_economy.execute(query, (user_id, server_id)).fetchone()
            result2 = self.db_cursor_user_items.execute(query2, (user_id, name, server_id)).fetchone()
            ...
            if result:
                ...
                if result2:
                    ...
                    if user_money >= item_price:
                        ...
                        query = "UPDATE economy SET money = ? WHERE user_id = ? AND server_id = ?"
                        self.db_cursor_economy.execute(query, (new_money, user_id, server_id))
                        self.db_connection_economy.commit()

                        # Add the purchased item to the user's inventory
                        query = "INSERT INTO user_items (user_id, item_name, server_id) VALUES (?, ?, ?)"
                        self.db_cursor_user_items.execute(query, (user_id, name, server_id))
                        self.db_connection_user_items.commit()
        ...
        if result:
            if item_name == "Bomb":
                ...
                if not timeout_role:
                    ...
                    self.db_cursor_user_items.execute(delete_query, (result["id"],))
                    self.db_connection_user_items.commit()
                    timeout_seconds = 300
    ...
    async def inventory(self, ctx, member: discord.Member = None):
        ...
        result = self.db_cursor_user_items.execute(query, (user_id, server_id)).fetchall()
        ...
        if result:
            ...
            embed = discord.Embed(title="Inventory", description="Your current inventory", color=discord.Color.blue())
